        ...
        <table class="w-full text-left text-sm text-gray-400">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-800 last:border-0 hover:bg-gray-800/20 transition-colors">
                    ...
                    <td class="px-6 py-4 text-gray-400 text-xs max-w-xs" title="{{ strip_tags($detail->detail_description) }}">
                         {{ \Illuminate\Support\Str::limit(strip_tags($detail->detail_description), 50) }}
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-4">
                            <a href="{{ route('admin.report_details.edit', $detail->id) }}" class="flex items-center justify-center w-8 h-8 rounded bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 hover:text-indigo-300 transition-colors border border-indigo-500/20 shadow-sm" title="Edit">
                            ...
                            <button type="button" class="flex items-center justify-center w-8 h-8 rounded bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 transition-colors border border-rose-500/20 shadow-sm" title="Delete" onclick="openDeleteModal('{{ $detail->id }}', '{{ addslashes(strip_tags($detail->title)) }}')">
