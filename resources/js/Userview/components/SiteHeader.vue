        ...
        <div class="bottom-actions">
          <div class="header-search-container">
            <label class="header-search" :class="{ 'search-active': isDropdownOpen }">
              <input type="search" placeholder="Search Report" v-model="searchQuery" @input="handleSearch" @focus="showDropdown" @blur="hideDropdownDelayed" @keydown.enter="submitSearch" />
            ...
            <div v-if="isDropdownOpen && (searchResults.length > 0 || isSearching || (searchQuery && searchQuery.length > 1))" class="search-dropdown-menu">
               <div v-if="isSearching" class="search-status">Searching for results...</div>
               <div v-else-if="searchResults.length === 0 && searchQuery.length > 1" class="search-status">No reports found for "{{ searchQuery }}".</div>
               <div v-else class="search-results-list">
                  <router-link 
                    v-for="report in searchResults" 
                    :key="report.id" 
                    :to="'/report/' + report.slug_url" 
                    class="search-result-item" 
                    @mousedown="forceNavigate('/report/' + report.slug_url)"
                  >
                  ...
                  <router-link 
                    :to="'/reports?search=' + encodeURIComponent(searchQuery)" 
                    class="view-all-link"
          ...
          <a class="call-button" href="tel:+919370940742">
