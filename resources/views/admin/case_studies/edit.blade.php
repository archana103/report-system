...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto w-full mx-auto">
    <div class="flex items-center justify-between mb-8 overflow-hidden rounded-2xl bg-gradient-to-r from-teal-600/10 to-transparent p-4 border-l-4 border-teal-500">
        <div>
            <h2 class="text-2xl font-bold text-white tracking-tight">Edit Case Study</h2>
            <p class="text-gray-400 text-sm mt-1">Update general information and media.</p>
        </div>
        <a href="{{ route('admin.case_studies.index') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-xl text-sm font-medium text-gray-300 hover:text-white shadow-sm transition-all focus:ring-2 focus:ring-gray-600 focus:outline-none">
    ...
    <form method="POST" action="{{ route('admin.case_studies.update', $caseStudy->id) }}" enctype="multipart/form-data" class="space-y-8 pb-12">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 bg-gray-900/40 rounded-3xl border border-gray-800 shadow-2xl backdrop-blur-sm">
            <div class="col-span-1 md:col-span-2">
                ...
                <input name="title" type="text" value="{{ old('title', $caseStudy->title) }}" placeholder="Case Study Title..." required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner" />
            </div>

            <div class="col-span-1">
                <label class="block text-sm font-semibold text-gray-300 mb-3 ml-1">URL <span class="text-rose-500">*</span></label>
                <input name="url" type="text" value="{{ old('url', $caseStudy->url) }}" placeholder="case-study-url-slug" required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner font-mono text-xs" />
            </div>
            ...
            <div class="col-span-1 md:col-span-2">
                ...
                <textarea name="description" rows="4" placeholder="Brief summary of the Case Study..." required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner">{{ old('description', $caseStudy->description) }}</textarea>
            </div>
            ...
            <div class="col-span-1">
                ...
                <div class="border-2 border-dashed border-gray-700 rounded-xl p-4 bg-gray-800/50 text-center hover:bg-gray-800 hover:border-gray-600 transition-all">
                    @if($caseStudy->main_image)
                        <div class="mb-4">
                            <span class="block text-xs text-gray-500 mb-2">Current Main Image</span>
                            <img src="{{ $caseStudy->main_image }}" class="w-full h-auto max-h-32 object-cover rounded-lg border border-gray-700 mx-auto" />
                        </div>
                    @endif
            ...
            <div class="col-span-1">
                ...
                <div class="border-2 border-dashed border-gray-700 rounded-xl p-4 bg-gray-800/50 text-center hover:bg-gray-800 hover:border-gray-600 transition-all">
                    @if($caseStudy->thumbnail_image)
                        <div class="mb-4">
                            <span class="block text-xs text-gray-500 mb-2">Current Thumbnail</span>
                            <img src="{{ $caseStudy->thumbnail_image }}" class="w-32 h-32 object-cover rounded-lg border border-gray-700 mx-auto" />
                        </div>
                    @endif
            ...
            <div class="col-span-1 md:col-span-2 pt-4 flex items-center justify-end space-x-4">
                <a href="{{ route('admin.case_studies.index') }}" class="px-8 py-3 rounded-xl text-sm font-medium text-gray-400 hover:text-white transition-all">Cancel</a>
