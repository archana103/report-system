...
@section('header_title', 'Case Studies Management')
...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-medium text-white tracking-tight">Case Studies List</h2>
        <a href="{{ route('admin.case_studies.create') }}" class="bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-400 hover:to-emerald-500 text-white font-medium py-2 px-6 rounded-xl shadow-lg transition-all">
            Add New Case Study
        </a>
    </div>
    ...
    <div class="mb-6 flex justify-end">
        <form method="GET" action="{{ route('admin.case_studies.index') }}" class="relative w-full max-w-sm">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search Case Studies..." class="w-full bg-gray-900/50 border border-gray-700 text-white pl-4 pr-10 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 transition-colors">
    ...
    <div class="overflow-x-auto bg-gray-900/50 rounded-2xl border border-gray-800 shadow-inner">
        <table class="w-full text-left text-sm text-gray-400">
            ...
            <tbody>
                @forelse ($caseStudies as $index => $pr)
                <tr class="border-b border-gray-700/50 last:border-0 hover:bg-[#1B2230]/50 transition-colors bg-[#151B26]">
                    <td class="px-5 py-4 text-gray-300 font-medium whitespace-nowrap">{{ $caseStudies->firstItem() + $index }}</td>
                    ...
                    <td class="px-5 py-4 text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('admin.case_studies.edit', $pr->id) }}" class="flex items-center justify-center w-8 h-8 rounded bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 hover:text-indigo-300 transition-colors border border-indigo-500/20 shadow-sm" title="Edit">
                ...
                @empty
                <tr>
                    <td colspan="8" class="px-6 py-8 text-center text-gray-500">No Case Studies found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="mt-6 flex justify-end relative z-10">
        {{ $caseStudies->links('pagination::tailwind') }}
    </div>
...
<div id="deleteModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/80 backdrop-blur-sm transition-opacity" style="opacity: 0;">
    <div class="bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden transform scale-95 transition-all duration-200" id="deleteModalPanel">
        <div class="p-6">
            ...
            <h3 class="text-lg font-medium text-gray-200 text-center mb-2">Delete Case Study</h3>
            <p class="text-sm text-gray-400 text-center" id="deleteModalMessage">Are you sure you want to delete this Case Study? This action cannot be undone.</p>
        </div>
...
<script>
    function openDeleteModal(id, title) {
        ...
        form.action = `{{ route('admin.case_studies.destroy', ':id') }}`.replace(':id', id);
