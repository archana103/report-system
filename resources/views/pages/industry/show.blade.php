...
<div class="industry-category-page">
    ...
    <section class="category-hero-banner" style="{{ !empty($categoryInfo->category_image) ? 'background-image: url(' . $categoryInfo->category_image . ');' : '' }}">
        ...
        <section class="category-reports-list">
            ...
            <div class="report-list-vertical">
              ...
              <article class="report-list-card">
                ...
                <div class="report-details">
                  <a href="{{ url('/report/' . (!empty($report->slug) && $report->slug !== '#' ? $report->slug : $report->id)) }}" class="report-title-link">
                  ...
                  <div class="report-metadata">
                    ...
                    <span class="divider">|</span>
                    <span>Publish Date: <strong>{{ explode('-', $report->date ?? '')[0] }} {{ explode('-', $report->date ?? '')[1] ?? '' }}</strong></span>
                  </div>
            ...
            <div class="pagination-wrapper" style="display: flex;">
                ...
                <a href="?page={{ request()->query('page', 1) - 1 }}" class="nav-btn prev-btn" style="text-decoration: none;">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="nav-icon" style="width:16px"><path d="m15 18-6-6 6-6"></path></svg>
                    Previous
                </a>
                @endif
                <div class="page-numbers">
                    ...
                    @endphp
                    @for ($i = $start; $i <= $end; $i++)
                        <a href="?page={{ $i }}" class="num-btn {{ $i == $currentPage ? 'active' : '' }}" style="text-decoration: none; display: flex; align-items: center; justify-content: center;">{{ $i }}</a>
                    @endfor
                </div>
                ...
                <a href="?page={{ request()->query('page', 1) + 1 }}" class="nav-btn next-btn" style="text-decoration: none;">
                    Next
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="nav-icon" style="width:16px"><path d="m9 18 6-6-6-6"></path></svg>
                </a>
                @endif
            </div>
            @endif
      ...
      <div class="cta-inner" style="background-image: url('{{ env('AWS_URL') }}/assets/images/background-image/reportpage_cta.png'); background-size: cover; background-position: center; background-repeat: no-repeat;">
        <div class="cta-content">
          ...
          <a href="/contact-us" class="primary-button cta-btn">
            Request Research 
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="icon" aria-hidden="true" style="width:16px">
