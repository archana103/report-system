...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto w-full mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-medium text-white tracking-tight">Add Blog</h2>
        <a href="{{ route('admin.blogs.index') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-xl text-sm font-medium text-gray-300 hover:text-white shadow-sm transition-all focus:ring-2 focus:ring-gray-600 focus:outline-none">
            &larr; Back to List
        </a>
    </div>
    ...
    <form method="POST" action="{{ route('admin.blogs.store') }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input name="title" type="text" value="{{ old('title') }}" required class="w-full bg-gray-800/80 border border-gray-700 rounded-xl px-4 py-2.5 text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-1.5 ml-1">Slug URL</label>
                <input name="url" type="text" value="{{ old('url') }}" class="w-full bg-gray-800/80 border border-gray-700 rounded-xl px-4 py-2.5 text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all" />
            </div>

            <!-- Row 2 -->
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-1.5 ml-1">Author Name</label>
                <input name="author_name" type="text" value="{{ old('author_name') }}" class="w-full bg-gray-800/80 border border-gray-700 rounded-xl px-4 py-2.5 text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all" />
            </div>
            <div>
        ...
        <div>
            ...
            <div class="rounded-xl overflow-hidden shadow-sm border border-gray-700 bg-gray-800/50 transition-all">
                <textarea name="description" class="w-full bg-transparent p-4 text-white focus:outline-none min-h-[160px]">{{ old('description') }}</textarea>
            </div>
        </div>
