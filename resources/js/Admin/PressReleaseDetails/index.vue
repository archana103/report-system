<script setup>
import { ref } from 'vue';
import AddPressReleaseDetail from './AddPressReleaseDetail.vue';
import ListPressReleaseDetail from './ListPressReleaseDetail.vue';
  ...
  <div class="min-h-[500px] bg-gray-800/40 rounded-3xl p-6 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-hidden flex flex-col">
    ...
    <div class="relative z-10 flex-shrink-0">
      ...
      <div class="flex space-x-4 mb-4 border-b border-gray-700/50 pb-2">
        <button
          @click="activeTab = 'list'; editingDetail = null"
          :class="[
        ...
        <button
          @click="activeTab = 'add'; editingDetail = null"
          :class="[
            'px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200',
            activeTab === 'add'
              ? 'bg-teal-500/20 text-teal-400 border border-teal-500/30'
        ...
        >
          Edit: {{ editingDetail?.press_release?.title }}
        </button>
      </div>
    </div>
    ...
    <div class="relative z-10 flex-grow bg-gray-900/50 rounded-2xl p-4 border border-gray-800 shadow-inner overflow-y-auto w-full">
      <ListPressReleaseDetail v-if="activeTab === 'list'" @edit="handleEdit" />
      <AddPressReleaseDetail v-if="activeTab === 'add'" mode="add" @saved="handleSaved" />
      <AddPressReleaseDetail v-if="activeTab === 'edit'" mode="edit" :detail="editingDetail" @saved="handleSaved" />
    </div>
  </div>
</template>
