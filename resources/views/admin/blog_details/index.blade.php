...
@section('header_title', 'Blog Details Management')
...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-medium text-white tracking-tight">Blog Details List</h2>
    </div>

    <!-- Search Form -->
    <div class="mb-6 flex justify-end">
        <form method="GET" action="{{ route('admin.blog_details.index') }}" class="relative w-full max-w-sm">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search details by title or parent blog..." class="w-full bg-gray-900/50 border border-gray-700 text-white pl-4 pr-10 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 transition-colors">
    ...
    <div class="overflow-x-auto bg-gray-900/50 rounded-2xl border border-gray-800 shadow-inner">
        <table class="w-full text-left text-sm text-gray-400">
            <thead class="text-xs text-gray-500 uppercase bg-gray-800/50 border-b border-gray-700">
                <tr>
                    <th scope="col" class="px-6 py-4 font-medium">Assigned Blog</th>
                    <th scope="col" class="px-6 py-4 font-medium">Title</th>
                    <th scope="col" class="px-6 py-4 font-medium">Breadcrumb Title</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-800 last:border-0 hover:bg-gray-800/20 transition-colors">
                    <td class="px-6 py-4 font-medium text-indigo-300">{{ Str::limit($detail->blog->title ?? 'N/A', 40) }}</td>
                    <td class="px-6 py-4 font-medium text-gray-200">{{ Str::limit($detail->title, 40) }}</td>
                    <td class="px-6 py-4 text-gray-400">
                        {{ $detail->breadcrumb_title ?? 'N/A' }}
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-4">
                            <a href="{{ route('admin.blog_details.edit', $detail->id) }}" class="flex items-center justify-center w-8 h-8 rounded bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 hover:text-indigo-300 transition-colors border border-indigo-500/20 shadow-sm" title="Edit">
                            ...
                            <button type="button" class="flex items-center justify-center w-8 h-8 rounded bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 transition-colors border border-rose-500/20 shadow-sm" title="Delete" onclick="openDeleteModal('{{ $detail->id }}', '{{ addslashes($detail->title) }}')">
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-6 py-8 text-center text-gray-500">No blog details found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="deleteModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/80 backdrop-blur-sm transition-opacity" style="opacity: 0;">
    <div class="bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden transform scale-95 transition-all duration-200" id="deleteModalPanel">
        <div class="p-6">
            ...
            <h3 class="text-lg font-medium text-gray-200 text-center mb-2">Delete Blog Detail</h3>
            <p class="text-sm text-gray-400 text-center" id="deleteModalMessage">Are you sure you want to delete this blog detail? This action cannot be undone.</p>
        </div>
...
<script>
    function openDeleteModal(id, title) {
        ...
        const formAction = `{{ route('admin.blog_details.destroy', ':id') }}`.replace(':id', id);
