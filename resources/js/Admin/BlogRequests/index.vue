...
import { getBlogRequests, deleteBlogRequest } from './api.js'
...
const fetchData = async (page = 1) => {
  ...
  try {
    ...
    const response = await getBlogRequests(params)
...
const confirmDelete = async () => {
  ...
  try {
    await deleteBlogRequest(selectedItem.value.id)
    showDeleteModal.value = false
    fetchData(pagination.value.current_page)
  } catch (error) {
    console.error('Error deleting blog request:', error)
  }
}
...
<template>
  <div>
      ...
      <div class="relative z-10 flex-grow flex flex-col">
        ...
        >
          <template #item-blog="{ item }">
            <span class="text-gray-300 font-medium text-sm line-clamp-1" v-if="item.blog">
              {{ item.blog.title }}
            </span>
            <span class="text-gray-500 italic text-sm" v-else>Deleted Blog</span>
          </template>
    ...
    <ConfirmationModal
      ...
      title="Delete Blog Request"
      :message="`Are you sure you want to delete the blog request from '${selectedItem?.full_name}'? This action cannot be undone.`"
          ...
          <div class="p-6 overflow-y-auto" v-if="selectedItem">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p class="text-gray-200 font-medium">{{ selectedItem.full_name || '—' }}</p>
              </div>
              <div>
              ...
              <div class="md:col-span-2">
                ...
                <div class="bg-gray-900/50 p-4 rounded-xl border border-gray-700/50 mt-2">
                  <p class="text-indigo-300 font-semibold text-sm leading-relaxed" v-if="selectedItem.blog">
                    {{ selectedItem.blog.title }}
                  </p>
                  <p class="text-gray-500 italic text-sm" v-else>Deleted Blog</p>
                </div>
              </div>
            </div>
          </div>
