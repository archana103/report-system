...
@section('header_title', 'Edit Case Study Detail')
...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto w-full mx-auto">
    <div class="flex items-center justify-between mb-8 overflow-hidden rounded-2xl bg-gradient-to-r from-teal-600/10 to-transparent p-4 border-l-4 border-teal-500">
        <div>
            <h2 class="text-2xl font-bold text-white tracking-tight">Edit Case Study Detail</h2>
            <p class="text-gray-400 text-sm mt-1">Edit: {{ $detail->CaseStudy->title ?? 'N/A' }}</p>
        </div>
        <a href="{{ route('admin.case_study_details.index') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-xl text-sm font-medium text-gray-300 hover:text-white shadow-sm transition-all focus:ring-2 focus:ring-gray-600 focus:outline-none">
            &larr; Back to List
        </a>
    </div>
    ...
    <form method="POST" action="{{ route('admin.case_study_details.update', $detail->id) }}" class="space-y-8 pb-12">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 bg-gray-900/40 rounded-3xl border border-gray-800 shadow-2xl backdrop-blur-sm">
            <input type="hidden" name="case_study_id" value="{{ $detail->case_study_id }}">
            ...
            <div class="col-span-1">
                <label class="block text-sm font-semibold text-gray-300 mb-3 ml-1">Select Case Study <span class="text-rose-500">*</span></label>
                <input type="text" disabled value="{{ $detail->CaseStudy->title ?? 'N/A' }}" class="w-full bg-gray-800/60 border border-gray-700/50 rounded-xl px-4 py-2.5 text-sm text-gray-400 cursor-not-allowed shadow-inner" />
            </div>
            ...
            <div class="col-span-1 md:col-span-2">
                <label class="block text-sm font-semibold text-gray-300 mb-3 ml-1">Case Study Content</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 bg-gray-900/40 rounded-3xl border border-gray-800 shadow-2xl backdrop-blur-sm">
            <div>
                <label class="block text-sm font-semibold text-gray-400 mb-2 ml-1">Case Study Meta Title</label>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-400 mb-2 ml-1">Case Study Meta Description</label>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-400 mb-2 ml-1">Case Study Meta Keywords</label>
        ...
        <div class="p-8 bg-gray-900/40 rounded-3xl border border-gray-800 shadow-2xl backdrop-blur-sm" id="hreflang-container">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-gray-300">Case Study Hreflang Tags</h3>
        ...
        <div class="flex items-center justify-end space-x-4 w-full pt-4">
            <a href="{{ route('admin.case_study_details.index') }}" class="px-8 py-3 rounded-xl text-sm font-medium text-gray-400 hover:text-white transition-all">Cancel</a>
