...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto w-full mx-auto">
    <div class="flex items-center justify-between mb-8 overflow-hidden rounded-2xl bg-gradient-to-r from-teal-600/10 to-transparent p-4 border-l-4 border-teal-500">
        <div>
            <h2 class="text-2xl font-bold text-white tracking-tight">Add New Case Study</h2>
            <p class="text-gray-400 text-sm mt-1">Create a new Case Study. This will automatically generate a blank detail page.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.case_studies.store') }}" enctype="multipart/form-data" class="space-y-8 pb-12">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 bg-gray-900/40 rounded-3xl border border-gray-800 shadow-2xl backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div class="col-span-1 md:col-span-2">
                ...
                <input name="title" type="text" value="{{ old('title') }}" placeholder="Case Study Title..." required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner" />
            </div>

            <div class="col-span-1">
                <label class="block text-sm font-semibold text-gray-300 mb-3 ml-1">URL <span class="text-rose-500">*</span></label>
                <input name="url" type="text" value="{{ old('url') }}" placeholder="case-study-url-slug" required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner font-mono text-xs" />
            </div>
            ...
            <div class="col-span-1 md:col-span-2">
                ...
                <textarea name="description" rows="4" placeholder="Brief summary of the Case Study..." required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner">{{ old('description') }}</textarea>
            </div>
