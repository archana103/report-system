...
@section('header_title', 'Case Study Details Management')
...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-medium text-white tracking-tight">Case Study Details List</h2>
    </div>

    <!-- Search Form -->
    <div class="mb-6 flex justify-end">
        <form method="GET" action="{{ route('admin.case_study_details.index') }}" class="relative w-full max-w-sm">
    ...
    <div class="overflow-x-auto bg-gray-900/50 rounded-2xl border border-gray-800 shadow-inner">
        <table class="w-full text-left text-sm text-gray-400">
            <thead class="text-xs text-gray-500 uppercase bg-gray-800/50 border-b border-gray-700">
                <tr>
                    <th scope="col" class="px-6 py-4 font-medium">Assigned Case Study</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-800 last:border-0 hover:bg-gray-800/20 transition-colors">
                    <td class="px-6 py-4 font-medium text-indigo-300">{{ Str::limit($detail->CaseStudy->title ?? 'N/A', 60) }}</td>
                    <td class="px-6 py-4">
                        {{ $detail->created_at->format('M d, Y') }}
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-4">
                            <a href="{{ route('admin.case_study_details.edit', $detail->id) }}" class="flex items-center justify-center w-8 h-8 rounded bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 hover:text-indigo-300 transition-colors border border-indigo-500/20 shadow-sm" title="Edit">
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-6 py-8 text-center text-gray-500">No Case Study details found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<script>
    function openDeleteModal(id, title) {
        ...
        const formAction = `{{ route('admin.case_study_details.destroy', ':id') }}`.replace(':id', id);
