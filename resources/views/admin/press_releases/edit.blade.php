...
@section('header_title', 'Edit Press Release')
...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto w-full mx-auto">
    ...
    <form method="POST" action="{{ route('admin.press_releases.update', $pressRelease->id) }}" enctype="multipart/form-data" class="space-y-8 pb-12">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 bg-gray-900/40 rounded-3xl border border-gray-800 shadow-2xl backdrop-blur-sm">
            <div class="col-span-1 md:col-span-2">
                ...
                <input name="title" type="text" value="{{ old('title', $pressRelease->title) }}" placeholder="Press Release Title..." required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner" />
            </div>

            <div class="col-span-1">
                <label class="block text-sm font-semibold text-gray-300 mb-3 ml-1">URL <span class="text-rose-500">*</span></label>
                <input name="url" type="text" value="{{ old('url', $pressRelease->url) }}" placeholder="press-release-url-slug" required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner font-mono text-xs" />
            </div>
            ...
            <div class="col-span-1">
                ...
                <select name="status" required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner">
                    <option value="Active" {{ old('status', $pressRelease->status) == 'Active' ? 'selected' : '' }}>Active</option>
                    <option value="Inactive" {{ old('status', $pressRelease->status) == 'Inactive' ? 'selected' : '' }}>Inactive</option>
                </select>
            </div>

            <div class="col-span-1 md:col-span-2">
                <label class="block text-sm font-semibold text-gray-300 mb-3 ml-1">Short Description <span class="text-rose-500">*</span></label>
                <textarea name="description" rows="4" placeholder="Brief summary of the press release..." required class="w-full bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 text-sm text-gray-200 focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all shadow-inner">{{ old('description', $pressRelease->description) }}</textarea>
            </div>
                ...
                <div class="border-2 border-dashed border-gray-700 rounded-xl p-4 bg-gray-800/50 text-center hover:bg-gray-800 hover:border-gray-600 transition-all">
                    @if($pressRelease->main_image)
                        <div class="mb-4">
                            ...
                            <img src="{{ $pressRelease->main_image }}" class="w-full h-auto max-h-32 object-cover rounded-lg border border-gray-700 mx-auto" />
                        </div>
                    @endif
                ...
                <div class="border-2 border-dashed border-gray-700 rounded-xl p-4 bg-gray-800/50 text-center hover:bg-gray-800 hover:border-gray-600 transition-all">
                    @if($pressRelease->thumbnail_image)
                        <div class="mb-4">
                            ...
                            <img src="{{ $pressRelease->thumbnail_image }}" class="w-32 h-32 object-cover rounded-lg border border-gray-700 mx-auto" />
                        </div>
                    @endif
