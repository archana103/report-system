...
@section('header_title', 'Report List')
...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-medium text-white tracking-tight">Report List</h2>
        <a href="{{ route('admin.reports.create') }}" class="bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-400 hover:to-emerald-500 text-white font-medium py-2 px-6 rounded-xl shadow-lg transition-all">
            Add New Report
        </a>
    </div>
    ...
    <div class="mb-6 flex justify-end">
        <form method="GET" action="{{ route('admin.reports.index') }}" class="flex flex-col sm:flex-row gap-3 w-full max-w-2xl justify-end">
            <select name="category_id" onchange="this.form.submit()" class="bg-gray-900/50 border border-gray-700 text-white pl-4 pr-10 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 transition-colors appearance-none cursor-pointer">
            ...
            <div class="relative w-full sm:w-64">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search reports..." class="w-full bg-gray-900/50 border border-gray-700 text-white pl-4 pr-10 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 transition-colors">
        ...
        <table class="w-full text-left text-sm text-gray-400">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-800 last:border-0 hover:bg-gray-800/20 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        @if(strtolower($report->status) === 'active' || $report->status == 1)
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-4">
                            <a href="{{ route('admin.reports.edit', $report->id) }}" class="flex items-center justify-center w-8 h-8 rounded bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 hover:text-indigo-300 transition-colors border border-indigo-500/20 shadow-sm" title="Edit">
                            ...
                            <button type="button" class="flex items-center justify-center w-8 h-8 rounded bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 transition-colors border border-rose-500/20 shadow-sm" title="Delete" onclick="openDeleteModal('{{ $report->id }}', '{{ addslashes($report->name) }}')">
...
<div id="deleteModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/80 backdrop-blur-sm transition-opacity" style="opacity: 0;">
    <div class="bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden transform scale-95 transition-all duration-200" id="deleteModalPanel">
        <div class="p-6">
            ...
            <h3 class="text-lg font-medium text-gray-200 text-center mb-2">Delete Report</h3>
            <p class="text-sm text-gray-400 text-center" id="deleteModalMessage">Are you sure you want to delete this report? This action cannot be undone.</p>
        </div>
...
<script>
    function openDeleteModal(reportId, reportName) {
        ...
        // Update Content
        message.innerText = `Are you sure you want to delete '${reportName}'? This action cannot be undone.`;
