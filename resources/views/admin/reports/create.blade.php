...
@section('header_title', 'Add New Report')
...
<div class="h-full bg-gray-800/40 rounded-3xl p-8 shadow-2xl border border-gray-700/50 backdrop-blur-sm relative overflow-y-auto w-full  mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-medium text-white tracking-tight">Add New Report</h2>
        <a href="{{ route('admin.reports.index') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-xl text-sm font-medium text-gray-300 hover:text-white shadow-sm transition-all focus:ring-2 focus:ring-gray-600 focus:outline-none">
            &larr; Back to List
        </a>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    >
                        <option value="" disabled {{ old('report_category_id') ? '' : 'selected' }}>-- Select Category --</option>
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('report_category_id') == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-300 mb-1.5 ml-1">
                    New Report Name <span class="text-rose-500">*</span>
                </label>
                <textarea
                    name="name"
                    rows="3"
